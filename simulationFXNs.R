...
makeParms <- function(
  ...
  lastPregRate = 0.001, #Taylor, 2003
...
#For each week, the participant is assigned a risk (totalRate) that is the product of their individual RR and their regional risk that week
...
#If a CZS trial, this gets conception time:
...
getCZSoutcome <- function(trial,parms,browse=F) with(parms, {
  ...
  if(trialType == 'CZStrial'){
    ...
    trial$trimesterInfected <- as.character(Inf)
...
persistence <- function(trial,parms,browse = F) with(parms, {
  if(browse) browser()
  Dates <- seq.Date(as.Date(startDate),as.Date(maxEnddate), by = testInterval)
  testDates <- list()
  ...
  for(ind in 1:nrow(trial)){
    if(trial[ind,'status'] == 1){
    trial[ind,'testResult']<-ifelse(any(in_interval(testDates,trial[ind,'firstDetectableBlood'],trial[ind,'lastDetectableBlood'])),1,0)
